...
reads<-read_biom("Kaiju Taxonomy Abundance/kaiju_feature-table-trimmed.biom")
OTUtable<-as.data.frame(as.matrix(biom_data(reads)))%>%
  rownames_to_column("Feature.ID")
taxa<-read.table("Kaiju Taxonomy Abundance/kaiju_taxonomy-trimmed.tsv",sep='\t',header = TRUE)
OTUtable2<-inner_join(OTUtable,taxa)%>%
  select(-"Feature.ID")%>%
  relocate(Taxon)%>%
  mutate(Taxon=str_replace_all(Taxon,".__",""))%>%
  separate(Taxon,c("Domain","Phylum","Class","Order","Family","Genus","Species"),sep=";")
write_tsv(OTUtable2,"Kaiju Taxonomy Abundance/kaiju_abun-trimmed.tsv")
